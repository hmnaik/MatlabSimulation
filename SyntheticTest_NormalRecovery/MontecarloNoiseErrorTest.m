...
% noisefile = 'noise100.mat';
% checkNoiseStatus = exist(noisefile);
% if( checkNoiseStatus == 0)   
%     noise = randn(1,samplePoints) ;
%     save(noisefile,'noise');
% end
% load(noisefile,'noise'); 
% 
% minNoise = min(noise);
% maxNoise = max(noise);
% 
% minNoiseRange = [ minNoise*noiseFactorRange(1) minNoise*noiseFactorRange(noiseSize)];
% maxNoiseRange = [ maxNoise*noiseFactorRange(1) maxNoise*noiseFactorRange(noiseSize)];
...
xlabel(' distance (mm) ');ylabel(' Angle (degree) ');zlabel(' Center estimation Error 2 (stdDiv - mm)');
subplot(1,2,2)
...
xlabel('distance (mm)');ylabel('Angle (degree)');zlabel('Angular Normal Error 1(stdDiv - Degree)');
subplot(1,2,2);
...
xlabel(' distance (mm) ');ylabel(' Angle (degree) ');zlabel('distance between C1-C2 (stdDiv - mm)');
subplot(1,2,2)
